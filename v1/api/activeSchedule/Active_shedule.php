...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['id'])) {
        try {
            ...
            $resetActive = $conn->query("UPDATE SSBaide_active_shedule SET Active = 0 WHERE Active = 1 AND id != $id");

            if (!$resetActive) {
                $res = array("status" => "F", "message" => "Error resetting Active column: " . $conn->error);
